...
def handle_get_data_sheet_action(payload, action):
    ...
    try:
        ...
        # Extract sheet info from action value
        value = action.get('value', '{}')
        try:
            sheet_info = json.loads(value)
            sheet_id = sheet_info.get('sheet_id')
            sheet_name = sheet_info.get('sheet_name', 'Unknown')
        except (json.JSONDecodeError, KeyError):
            logger.error("Failed to parse sheet info from action value")
            return response
        ...
        def fetch_and_send_data():
            try:
                ...
                if not data:
                    slack_service.post_message(
                        channel=user_id,
                        text=f"📝 No data found in sheet: {sheet_name}",
                        ephemeral=True
                    )
                    return
                
                # Format and send response
                blocks = slack_service.format_sheet_data_blocks(data, sheet_id, "A1:Z10")
                slack_service.post_message(
                    channel=user_id,
                    blocks=blocks,
                    ephemeral=True
    ...
    try:
        ...
        def refresh_data():
            try:
                ...
                logger.error(f"Error refreshing sheet data: {e}")
                slack_service.post_message(
                    channel=user_id,
                    text=f"❌ Error refreshing data from sheet: {str(e)}",
                    ephemeral=True
                )
...
def handle_open_update_modal_action(payload, action):
    ...
    try:
        ...
        # Build and open modal
        sheet_id = params.get('sheet_id') if source == 'sheet' else None
        modal = slack_service.build_write_data_modal(source, sheet_id)
        slack_service.open_modal(trigger_id, modal)
...
def handle_view_submission(payload):
    ...
    try:
        ...
        if callback_id == 'write_data_modal' or callback_id == 'write_sheet_data_modal':
...
def handle_write_data_modal_submission(payload):
    ...
    try:
        ...
        # Extract sheet_id from private_metadata
        ...
        try:
            ...
            logger.error("Failed to parse private_metadata or missing sheet_id")
            return jsonify({
        ...
        # Start async operation
        thread = threading.Thread(target=update_sheet_async)
        thread.daemon = True
        thread.start()
